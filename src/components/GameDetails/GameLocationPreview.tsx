...
export function GameLocationPreview({ placeName, address, latitude, longitude }: Props) {
  const openMaps = () => {
  ...
  return (
    <View style={styles.wrap}>
      ...
      {!!address?.trim() && <Text style={styles.address}>{address.trim()}</Text>}
...
const styles = StyleSheet.create({
  ...
  place: { fontSize: 16, fontWeight: "800", color: "#6A6A6A" },
  address: { marginTop: 6, fontSize: 14, color: "#8B8EA1" },
  ...
  openHintText: { color: "#fff", fontWeight: "900", fontSize: 12 },
});